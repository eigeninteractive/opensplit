...
class Share {
  ...
  @override
  String toString() => 'Share[memberId=$memberId, amountMinor=$amountMinor, weightMicros=$weightMicros]';
  ...
  static Share? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'memberId'), 'Required key "Share[memberId]" is missing from JSON.');
        assert(json[r'memberId'] != null, 'Required key "Share[memberId]" has a null value in JSON.');
        assert(json.containsKey(r'amountMinor'), 'Required key "Share[amountMinor]" is missing from JSON.');
        assert(json[r'amountMinor'] != null, 'Required key "Share[amountMinor]" has a null value in JSON.');
        return true;
      }());
