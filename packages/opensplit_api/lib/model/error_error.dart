...
class ErrorError {
  ...
  static ErrorError? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'code'), 'Required key "ErrorError[code]" is missing from JSON.');
        assert(json[r'code'] != null, 'Required key "ErrorError[code]" has a null value in JSON.');
        assert(json.containsKey(r'message'), 'Required key "ErrorError[message]" is missing from JSON.');
        assert(json[r'message'] != null, 'Required key "ErrorError[message]" has a null value in JSON.');
        assert(json.containsKey(r'retry'), 'Required key "ErrorError[retry]" is missing from JSON.');
        assert(json[r'retry'] != null, 'Required key "ErrorError[retry]" has a null value in JSON.');
        return true;
      }());
