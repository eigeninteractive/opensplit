...
class Member {
  ...
  @override
  String toString() =>
      'Member[id=$id, profileId=$profileId, displayName=$displayName, upiVpa=$upiVpa, joinedAt=$joinedAt, leftAt=$leftAt, updatedAt=$updatedAt, seq=$seq]';
    ...
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'id'),
            'Required key "Member[id]" is missing from JSON.');
        assert(json[r'id'] != null,
            'Required key "Member[id]" has a null value in JSON.');
        assert(json.containsKey(r'profileId'),
            'Required key "Member[profileId]" is missing from JSON.');
        assert(json.containsKey(r'displayName'),
            'Required key "Member[displayName]" is missing from JSON.');
        assert(json[r'displayName'] != null,
            'Required key "Member[displayName]" has a null value in JSON.');
        assert(json.containsKey(r'upiVpa'),
            'Required key "Member[upiVpa]" is missing from JSON.');
        assert(json.containsKey(r'joinedAt'),
            'Required key "Member[joinedAt]" is missing from JSON.');
        assert(json[r'joinedAt'] != null,
            'Required key "Member[joinedAt]" has a null value in JSON.');
        assert(json.containsKey(r'leftAt'),
            'Required key "Member[leftAt]" is missing from JSON.');
        assert(json.containsKey(r'updatedAt'),
            'Required key "Member[updatedAt]" is missing from JSON.');
        assert(json[r'updatedAt'] != null,
            'Required key "Member[updatedAt]" has a null value in JSON.');
        assert(json.containsKey(r'seq'),
            'Required key "Member[seq]" is missing from JSON.');
        assert(json[r'seq'] != null,
            'Required key "Member[seq]" has a null value in JSON.');
        return true;
      }());
  ...
  static Map<String, List<Member>> mapListFromJson(
    ...
    if (json is Map && json.isNotEmpty) {
      ...
      for (final entry in json.entries) {
        map[entry.key] = Member.listFromJson(
          entry.value,
          growable: growable,
        );
      }
