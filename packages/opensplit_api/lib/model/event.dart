...
class Event {
  ...
  @override
  String toString() =>
      'Event[id=$id, actorId=$actorId, createdAt=$createdAt, kind=$kind, subjectId=$subjectId, payload=$payload, seq=$seq]';
  ...
  static Event? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        ...
        assert(json.containsKey(r'actorId'),
            'Required key "Event[actorId]" is missing from JSON.');
        assert(json.containsKey(r'createdAt'),
            'Required key "Event[createdAt]" is missing from JSON.');
        assert(json[r'createdAt'] != null,
            'Required key "Event[createdAt]" has a null value in JSON.');
        assert(json.containsKey(r'kind'),
            'Required key "Event[kind]" is missing from JSON.');
        assert(json[r'kind'] != null,
            'Required key "Event[kind]" has a null value in JSON.');
        assert(json.containsKey(r'subjectId'),
            'Required key "Event[subjectId]" is missing from JSON.');
        assert(json.containsKey(r'seq'),
            'Required key "Event[seq]" is missing from JSON.');
        assert(json[r'seq'] != null,
            'Required key "Event[seq]" has a null value in JSON.');
        return true;
      }());
